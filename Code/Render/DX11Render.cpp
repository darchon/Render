...
DX11Render::DX11Render(const DX11Render& other)
{
}
...
{
	//set swap chain to windowed before shutdown to avoid exceptions
	if(m_swapChain)
...
void DX11Render::BeginScene(float red, float green, float blue, float alpha)
{
	float color[4];

	//setup color of buffer
	color[0] = red;
	color[1] = green;
	color[2] = blue;
	color[3] = alpha;
...
void DX11Render::GetVideoCardInfo(char* cardName, int& memory)
{
